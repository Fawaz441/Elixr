...
def add_suit_to_cart(request,slug):
    ...
    if order_qs.exists():
        ...
        if order.ord_suits.filter(suit__slug=suit.slug).exists():
            ...
            ord_suit.quantity+=1
            ord_suit.save()
            messages.success(request,'Added {} of {} to cart'.format(ord_suit.quantity,ord_suit.suit.name))
            ...
            order.save()
            messages.success(request,"{} added to cart!".format(ordered_suit.suit.name))
            return redirect("suit_detail",slug=suit.slug)
...
def add_ship_to_cart(request,slug):
    ...
    if order_qs.exists():
        ...
        if order.ord_ships.filter(ship__slug=ship.slug).exists():
            ...
            ord_ship.quantity+=1
            ord_ship.save()
            messages.success(request,'Added {} of {} to cart'.format(ord_ship.quantity,ord_ship.ship.name))
            ...
            order.save()
            messages.success(request,"{} added to cart!".format(ordered_ship.ship.name))
            return redirect("ship_detail", slug=ship.slug)
...
def remove_ship_finally(request,slug):
    ...
    if order_qs.exists():
        ...
        if order.ord_ships.filter(ship__slug=ship.slug).exists():
            ...
            order.save()
            messages.success(request,'Removed {} from cart'.format(ord_ship.ship.name))
...
def remove_suit_finally(request,slug):
    ...
    if order_qs.exists():
        ...
        if order.ord_suits.filter(suit__slug=suit.slug).exists():
            ...
            order.save()
            messages.success(request,'Removed {} from cart'.format(ord_suit.suit.name))
            return redirect("cart")
    else:
        messages.warning(request,"You do not have an order")
...
def order_view(request):
    order_qs = Order.objects.filter(buyer=request.user,ordered=False)
    undelivered_order = Order.objects.filter(buyer=request.user,ordered=True,delivered=False)
